...
/* Action */
function drawActionDeco(color, position, lenNo) {
    context.fillStyle = color;

    drawActionBar(position, lenNo);
    // context.beginPath();
...
function drawActionBar(position, lenNo) {
    var lenTarget = 60;
    if (lenNo === 1) {
        lenTarget = 65;
    } else if (lenNo === 2) {
    ...
    if (position === 1) {
        context.arc(canvas.width/2, canvas.height/2 - 100, RADIUS/2 + 20, ((Math.PI / 180) * 220), ((Math.PI / 180) * 215), true);
        context.arc(canvas.width/2, canvas.height/2 - 100, RADIUS/2 + lenTarget, ((Math.PI / 180) * 215), ((Math.PI / 180) * 220), false);
    } else if (position === 2) {
        context.arc(canvas.width/2, canvas.height/2 - 100, RADIUS/2 + 20, ((Math.PI / 180) * 230), ((Math.PI / 180) * 225), true);
        context.arc(canvas.width/2, canvas.height/2 - 100, RADIUS/2 + lenTarget, ((Math.PI / 180) * 225), ((Math.PI / 180) * 230), false);
    } else if (position === 3) {
        context.arc(canvas.width/2, canvas.height/2 - 100, RADIUS/2 + 20, ((Math.PI / 180) * 240), ((Math.PI / 180) * 235), true);
        context.arc(canvas.width/2, canvas.height/2 - 100, RADIUS/2 + lenTarget, ((Math.PI / 180) * 235), ((Math.PI / 180) * 240), false);
    }
...
function animate() {
    ...
    drawTube();
    drawCheer('silver', x, y);
    drawCheer('gold', x + 500, y - 50);

    drawActionDeco('red', 1, Math.ceil(Math.random() * 3));
    drawActionDeco('green', 2, Math.ceil(Math.random() * 3));
    drawActionDeco('blue', 3, Math.ceil(Math.random() * 3));


    drawTeeth(canvas.width/2 - 105, canvas.height/2 - ((Math.random() * 4) + 50));
    drawTeeth(canvas.width/2 - 75, canvas.height/2 - ((Math.random() * 6) + 47));
    drawTeeth(canvas.width/2 - 45, canvas.height/2 - ((Math.random() * 6) + 47));
    drawTeeth(canvas.width/2 - 15, canvas.height/2 - ((Math.random() * 6) + 47));
    drawTeeth(canvas.width/2 + 15, canvas.height/2 - ((Math.random() * 6) + 47));
    drawTeeth(canvas.width/2 + 45, canvas.height/2 - ((Math.random() * 6) + 47));
    drawTeeth(canvas.width/2 + 75, canvas.height/2 - ((Math.random() * 4) + 50));

    drawMouth();

    if (x + radius > 180 || x - radius < 10) {
        dx = -dx;
    }

    if (y + radius > 280 || y - radius < 110) {
        dy = -dy;
    }
    x += dx;
    y += dy;
